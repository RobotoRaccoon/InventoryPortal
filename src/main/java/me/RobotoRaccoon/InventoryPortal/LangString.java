...
public class LangString {
    ...
    private String getLang(String key) {
        FileConfiguration config = InventoryPortal.getConfiguration().getLang();
        if (!config.contains(key)) {
            InventoryPortal.getPlugin().getLogger().warning(String.format("lang.yml path `%s` does not exist!", key));
            return key;
        }
